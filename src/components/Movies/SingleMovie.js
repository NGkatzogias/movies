...
export default function SingleMovie() {
  ...
  const { id } = useParams();
  const { movieDetails, allGenres } = useSingleMovieDetails(id);
  const { allTrailers, firstTrailer } = useMovieTrailers(id);
  const { reviews } = useGetMovieReviews(id);
  ...
  return (
    <div className="movie-details">
      ...
      <img
        onLoad={() => setPosterHeight(poster.height)}
      ...
      <button className="movie-details--more-videos" onClick={showMoreVideos}>
        {allTrailers.length} Videos
      </button>
      {showAllTrailers && (
          ...
          {reviews.map((review) => {
            return (
              <div className="review" key={review.id}>
                ...
                <p className="review--user-comment">{review.content}</p>
              </div>
            );
          })}
        </div>
